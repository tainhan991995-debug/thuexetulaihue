      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-8 px-5 lg:grid-cols-[1fr_1fr] lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <p className="mt-7 max-w-[640px] text-lg leading-[1.8] text-slate-200 sm:text-xl">
            Lựa chọn xe điện và xe xăng chất lượng, giao nhận tận nơi tại Huế. Đặt xe nhanh qua Hotline hoặc Zalo.
          </p>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.08 }} className="relative mx-auto w-full max-w-[720px] lg:ml-auto lg:mr-0">
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-white/30 bg-white/10 p-2 shadow-[0_35px_80px_rgba(0,0,0,.3)] backdrop-blur-sm">
            <img src="/images/tram-nha-minh-fleet.png" alt={`Đội xe ${brandName} tại Huế`} className="block h-auto w-full rounded-[1.6rem]" />
...
function CarImage({ car, className = "" }: { car: Car; className?: string }) {
  return <img src={car.image} alt={car.name} className={`h-full w-full object-contain ${className}`} style={{ objectPosition: car.position }} />;
}
...
function Pricing() {
  ...
  return <section id="bang-gia" className="scroll-mt-24 bg-ink py-20 text-white sm:py-28"><div className="mx-auto grid max-w-7xl gap-12 px-5 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:px-8"><div><p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[.18em] text-sand"><span className="h-px w-8 bg-sand" /> Bảng giá tham khảo</p><h2 className="mt-4 font-heading text-3xl font-bold sm:text-4xl">Rõ giá trước khi lên đường.</h2><p className="mt-5 max-w-md leading-7 text-slate-300">Giá thuê được lấy trực tiếp từ danh sách xe của {brandName}. Giá chính xác sẽ được xác nhận theo lịch nhận xe của bạn.</p><a href={zaloHref} target="_blank" rel="noreferrer" className="mt-7 inline-flex items-center gap-2 rounded-xl bg-sand px-5 py-3 text-sm font-bold text-ink transition hover:bg-[#f8b576]"><MessageCircle size={17} /> Nhắn Zalo đặt xe</a></div><div className="rounded-[1.75rem] border border-white/10 bg-white/10 p-5 backdrop-blur-sm sm:p-7"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-white/10 p-4"><span className="block text-[10px] font-bold uppercase tracking-wider text-slate-300">GIÁ XE CHỈ TỪ</span><strong className="mt-2 block text-sm text-white">{formatPrice(lowestPrice)}</strong></div><div className="rounded-xl bg-white/10 p-4"><span className="block text-[10px] font-bold uppercase tracking-wider text-slate-300">Giá cao nhất</span><strong className="mt-2 block text-sm text-white">{formatPrice(highestPrice)}</strong></div><div className="rounded-xl bg-white/10 p-4"><span className="block text-[10px] font-bold uppercase tracking-wider text-slate-300">Dòng xe</span><strong className="mt-2 block text-sm text-white">{cars.length} mẫu xe</strong></div></div><div className="mt-4 rounded-2xl bg-white p-5 text-ink"><div className="flex items-center justify-between border-b border-slate-100 pb-4"><span className="text-sm text-slate-500">Mức giá áp dụng</span><strong>Theo ngày thuê</strong></div><div className="flex items-center justify-between border-b border-slate-100 py-4"><span className="text-sm text-slate-500">Tình trạng xe</span><strong>Liên hệ xác nhận</strong></div><div className="flex items-center justify-between pt-4"><span className="font-heading text-lg font-bold">Đặt xe nhanh</span><strong className="font-heading text-xl text-teal">Zalo {hotline}</strong></div></div></div></div></section>;
}

function RentalNotice() {
  return <section className="bg-white py-20 sm:py-28"><div className="mx-auto grid max-w-7xl gap-10 px-5 lg:grid-cols-[.72fr_1.28fr] lg:items-center lg:px-8"><div><SectionTitle eyebrow="Thông tin cần biết" title="Lưu ý khi thuê xe." text="Vui lòng xem kỹ các lưu ý dưới đây trước khi nhận xe để chuyến đi diễn ra thuận lợi." /><a href="/images/luuy.jpg" target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-teal hover:text-ink">Mở ảnh lưu ý cỡ lớn <ArrowRight size={17} /></a></div><div className="overflow-hidden rounded-[1.75rem] border border-slate-100 bg-mist p-2 shadow-soft"><img src="/images/luuy.jpg" alt="Các lưu ý cần thiết khi thuê xe tại TRẠM NHÀ MINT" loading="lazy" className="h-auto w-full rounded-[1.35rem] object-contain" /></div></div></section>;
}
...
function Gallery() {
  ...
  const activeContent = activePanel === "gallery" ? <GalleryCarousel images={galleryImages} label={`Ảnh đội xe ${brandName}`} emptyMessage="Thêm ảnh vào thư mục public/images/gallery." /> : <ReelsPlayer reels={reelVideos} />;

  return <section id="gallery" className="bg-white py-20 sm:py-28"><div className="mx-auto max-w-7xl px-5 lg:px-8"><SectionTitle eyebrow="Hình ảnh đội xe" title="Những lựa chọn cho hành trình tại Huế." /><div className="mt-10 grid gap-4 lg:grid-cols-[1.35fr_.65fr]"><div>{activeContent}</div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1"><GalleryCard title="Thư viện ảnh" icon={MapPin} active={activePanel === "gallery"} onClick={() => setActivePanel("gallery")} /><GalleryCard title="Video / Reels" icon={Star} active={activePanel === "reels"} onClick={() => setActivePanel("reels")} /></div></div></div></section>;
}
...
export default function Home() {
  ...
  return <><Header /><main><Hero onSearch={goToCars} /><WhyUs /><CarsSection /><Pricing /><RentalNotice /><Gallery /><ProcessFaq /><Contact /></main><Footer /><FloatingActions /></>;
}
