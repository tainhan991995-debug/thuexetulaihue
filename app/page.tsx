      ...
      <div className="mx-auto flex h-[82px] max-w-7xl items-center justify-between px-5 md:h-[94px] lg:px-8">
        <button onClick={() => scrollToId("#trang-chu")} className="group flex items-center gap-3.5 text-left" aria-label="TRẠM NHÀ MINT – Về trang chủ">
          <span className="grid h-12 w-12 place-items-center overflow-hidden rounded-2xl bg-white shadow-float transition-transform group-hover:-rotate-3 md:h-[60px] md:w-[60px]">
            <Image src="/images/logo.jpg" width={48} height={48} alt="Logo TRẠM NHÀ MINT" className="h-full w-full object-contain" />
          </span>
          <span>
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-8 px-5 lg:grid-cols-[1fr_1fr] lg:px-8">
        <div>
          ...
          <p className="mt-7 max-w-[640px] text-base leading-7 text-slate-200 sm:text-xl sm:leading-[1.8]">
            Lựa chọn xe điện tự lái và xe xăng tự lái chất lượng cho chuyến thuê xe du lịch, giao xe tận nơi Huế theo lịch xác nhận. Đặt xe nhanh qua Hotline hoặc Zalo.
          </p>
...
function ReelsPlayer({ reels }: { reels: string[] }) {
  ...
  return <div className="overflow-hidden rounded-[1.75rem] bg-slate-950"><div className="relative h-[430px] bg-black"><video ref={videoRef} key={activeVideo} controls playsInline preload="none" poster={reelPosters[activeVideo] ?? "/images/tram-nha-minh-fleet.webp"} aria-label={`Video đội xe ${activeReel + 1}`} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onEnded={() => setIsPlaying(false)} className="h-full w-full bg-black object-contain"><source src={activeVideo} type="video/mp4" />Trình duyệt của bạn không hỗ trợ video.</video>{!isPlaying && <button type="button" onClick={togglePlayback} aria-label="Phát video" className="absolute inset-0 z-10 flex cursor-pointer items-center justify-center focus:outline-none focus-visible:ring-4 focus-visible:ring-white/80"><span className="grid h-20 w-20 place-items-center rounded-full bg-white/90 text-ink shadow-[0_10px_30px_rgba(0,0,0,.35)] backdrop-blur transition duration-200 hover:scale-110 hover:bg-teal hover:text-white sm:h-24 sm:w-24"><Play size={38} fill="currentColor" className="ml-1" /></span><span className="sr-only">Chạm để phát video</span></button>}</div>{reels.length > 1 && <div className="flex gap-2 overflow-x-auto bg-ink p-3">{reels.map((video, index) => <button key={video} onClick={() => selectReel(index)} className={`shrink-0 rounded-lg px-3 py-2 text-xs font-bold transition ${activeReel === index ? "bg-sand text-ink" : "bg-white/10 text-white hover:bg-white/20"}`}>Video {index + 1}</button>)}</div>}</div>;
}
