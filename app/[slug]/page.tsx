...
import { ArrowRight, ChevronRight, MapPin, MessageCircle, Phone } from "lucide-react";
...
export default async function SeoLandingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const serviceLinks = Array.from(new Set([...page.related, ...seoPages.map(({ slug: serviceSlug }) => serviceSlug)])).filter((serviceSlug) => serviceSlug !== page.slug).slice(0, 5).map((serviceSlug) => seoPageBySlug[serviceSlug]);
  const blogLinks = [...blogPosts].sort((a, b) => Number(b.serviceLinks.includes(page.slug)) - Number(a.serviceLinks.includes(page.slug))).slice(0, 5);
  const midwaySectionIndex = Math.floor((page.sections.length - 1) / 2);

  return <><SiteHeader /><main><Breadcrumb label={page.label} /><section className="pb-14 pt-8 sm:pb-20"><div className="mx-auto grid max-w-7xl gap-10 px-5 lg:grid-cols-[1fr_.92fr] lg:items-center lg:px-8"><div><p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[.18em] text-teal"><span className="h-px w-8 bg-sand" /> TRẠM NHÀ MINT · Huế</p><h1 className="mt-4 max-w-3xl font-heading text-4xl font-black leading-[1.1] tracking-[-.03em] text-ink sm:text-5xl">{page.h1}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">{page.lead}</p><div className="mt-7 flex flex-wrap gap-3"><a href={zaloHref} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-teal px-5 py-3.5 text-sm font-bold text-white transition hover:bg-[#207a70]"><MessageCircle size={18} /> Nhắn Zalo đặt xe</a><Link href="/bang-gia" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-5 py-3.5 text-sm font-bold text-ink transition hover:border-teal hover:text-teal">Xem bảng giá <ArrowRight size={17} /></Link></div></div><div className="overflow-hidden rounded-[2rem] border border-slate-100 bg-mist p-2 shadow-soft"><Image src={page.image} width={960} height={640} sizes="(min-width: 1024px) 46vw, 100vw" alt={page.imageAlt} className="h-[330px] w-full rounded-[1.55rem] object-cover object-center sm:h-[410px]" /></div></div></section><article className="bg-white py-16 sm:py-20"><div className="mx-auto grid max-w-7xl gap-12 px-5 lg:grid-cols-[minmax(0,1fr)_300px] lg:px-8"><div className="min-w-0"><div className="space-y-12">{page.sections.map((section, index) => <div key={section.heading}><section><h2 className="font-heading text-2xl font-bold leading-tight text-ink sm:text-3xl">{section.heading}</h2><div className="mt-5 space-y-5 text-[16px] leading-8 text-slate-600">{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></section>{index === midwaySectionIndex && <div className="mt-12"><BookingCta placement="middle" /></div>}</div>)}</div><section className="mt-14" id="cau-hoi"><p className="text-xs font-extrabold uppercase tracking-[.18em] text-teal">Câu hỏi thường gặp</p><h2 className="mt-3 font-heading text-3xl font-bold text-ink">Giải đáp về {page.label.toLowerCase()}</h2><div className="mt-7 divide-y divide-slate-100 overflow-hidden rounded-[1.5rem] border border-slate-100 bg-white shadow-soft">{page.faqs.map((faq) => <details key={faq.question} className="group p-5"><summary className="cursor-pointer list-none font-heading font-bold text-ink">{faq.question}</summary><p className="mt-3 leading-7 text-slate-600">{faq.answer}</p></details>)}</div></section><div className="mt-14"><BookingCta placement="end" /></div></div><aside className="h-fit rounded-[1.5rem] bg-mist p-6 lg:sticky lg:top-6"><MapPin size={22} className="text-teal" /><h2 className="mt-4 font-heading text-xl font-bold text-ink">Khám phá thêm</h2><p className="mt-2 text-sm leading-6 text-slate-500">So sánh các dịch vụ và đọc hướng dẫn để chuẩn bị lịch thuê phù hợp.</p><div className="mt-5 grid gap-2">{serviceLinks.map((service) => <Link key={service.slug} href={`/${service.slug}`} className="flex items-center justify-between rounded-xl bg-white px-4 py-3 text-sm font-bold text-ink shadow-sm transition hover:bg-teal hover:text-white">{service.label}<ArrowRight size={16} /></Link>)}</div><div className="mt-7 border-t border-slate-200 pt-6"><p className="text-xs font-extrabold uppercase tracking-[.14em] text-teal">Hướng dẫn liên quan</p><div className="mt-3 grid gap-2">{blogLinks.map((post) => <Link key={post.slug} href={`/blog/${post.slug}`} className="rounded-xl bg-white px-4 py-3 text-sm font-bold leading-5 text-ink shadow-sm transition hover:text-teal">{post.title}</Link>)}</div></div><Link href="/lien-he" className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-teal hover:text-ink">Liên hệ TRẠM NHÀ MINT <ArrowRight size={16} /></Link></aside></div></article></main><footer className="border-t border-slate-100 bg-white"><div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-9 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between lg:px-8"><Link href="/" className="flex items-center gap-3 font-heading font-extrabold text-ink"><span className="grid h-10 w-10 place-items-center overflow-hidden rounded-xl bg-white shadow-soft"><Image src="/images/logo.webp" width={40} height={40} alt="Logo TRẠM NHÀ MINT" className="h-full w-full object-contain" /></span> TRẠM NHÀ MINT</Link><div className="flex flex-wrap gap-x-5 gap-y-2"><Link href="/thue-xe-tu-lai-hue">Thuê xe tự lái Huế</Link><Link href="/bang-gia">Bảng giá</Link><Link href="/faq">FAQ</Link><Link href="/lien-he">Liên hệ</Link></div></div></footer><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(pageSchema) }} /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbSchema) }} /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }} /></>;
}
