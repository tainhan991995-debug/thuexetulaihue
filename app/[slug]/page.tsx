...
export default async function SeoLandingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!page) notFound();
  const canonical = `${siteUrl}/${page.slug}`;
  const articleSchema = { "@context": "https://schema.org", "@type": "Article", headline: page.h1, description: page.description, mainEntityOfPage: canonical, image: `${siteUrl}/og.png`, author: { "@type": "Organization", name: "TRẠM NHÀ MINT" }, publisher: { "@type": "Organization", name: "TRẠM NHÀ MINT" } };
